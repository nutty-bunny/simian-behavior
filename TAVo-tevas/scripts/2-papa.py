...
base = "/Users/similovesyou/Desktop/qts/simian-behavior/TAVo-tevas"
tavo_tevas = os.path.join(directory, 'tav.pickle')
with open(tavo_tevas, 'rb') as handle:
...
selected_monkeys = ["barnabe", "horus", "gandhi", "hercules", "iron", "lassa", "walt", "jipsy", "olga"]

# Define rolling windows
rolling_windows = [1, 10, 20, 30]

for window_size in rolling_windows:
    print(f"Creating plot with rolling window: {window_size}")
    ...
    for i, monkey in enumerate(selected_monkeys):
        ...
        for species, content in species_data.items():
            ...
            stacked_df = stacked_df.rolling(window=window_size, min_periods=1, center=True).mean()
    ...
    # Add rolling window info to the main title
    ...
    output_dir = os.path.join(base, "plots", "roll-all-over-me-no-hierarchy")
    os.makedirs(output_dir, exist_ok=True)
    fig.savefig(os.path.join(output_dir, f"monkeys_3x3_window{window_size}.svg"), format="svg")
    fig.savefig(os.path.join(output_dir, f"monkeys_3x3_window{window_size}.png"), format="png", dpi=300)
