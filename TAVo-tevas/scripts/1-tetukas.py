                ...
                if step_file and attempt_file:
                    try:
                        ...
                        if 'task' in attempts.columns and 'level' in attempts.columns:
                            task_filter = attempts[attempts['task'].isin([125])]
                            ...
                            if not task_filter.empty:
                                ...
                                if len(session_counts) < 10:
                                    print(f"Skipping '{species}' '{name}' - less than 10 unique sessions") # aribitrary, re-consider using attempts!!!
                                ...
                                print(f"Skipping '{species}' '{name}' - no data for task id 125")
                                del data[species][name]
                        else:
...
with open(tavo_tevas, 'wb') as handle: 
    pickle.dump(data, handle, protocol=pickle.HIGHEST_PROTOCOL)

print(f"data saved to {tavo_tevas}")
