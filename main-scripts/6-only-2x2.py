...
def run_focused_gee_analysis(final_data, result_types):
    ...
    # models focused on the research question (with species as covariate)
    models_to_fit = {
...
def select_best_models_and_report(results, alpha=0.05):
    ...
    for outcome in results:
        ...
        if valid_models:
            # Compare AICs
            aic_comparison = {k: v.aic for k, v in valid_models.items()}
            ...
            print(f"  Best model: {best_model_name} (AIC = {best_model.aic:.2f})")
            for name, aic in sorted(aic_comparison.items(), key=lambda x: x[1]):
                delta_aic = aic - best_model.aic
                print(f"    {name}: AIC = {aic:.2f} (Δ = {delta_aic:.2f})")
...
def create_social_status_individual_plots(results, final_data, best_models):
    ...
    for idx, outcome in enumerate(outcome_order):
        ...
        if outcome not in best_models:
            ax.text(0.5, 0.5, 'No model available', ha='center', va='center', 
                   fontsize=14, transform=ax.transAxes, color='gray')
            ax.set_title(outcome_labels[outcome], fontsize=16, fontweight='bold')
            continue
        ...
        if model_name == 'social_status_age_interaction' and 'elo_score_z:age' in coefs.index:
            ...
            for i, age in enumerate(age_quartiles):
                ...
                ax.plot(elo_range, prob_vals, color=colors[i], linewidth=3,
                       label=f"{labels[i]} ({age:.1f}y)", alpha=0.8)
                ax.fill_between(elo_range, prob_vals, alpha=0.15, color=colors[i])
            
            ax.legend(fontsize=11, loc='best', framealpha=0.9, title='Age Group')
        ...
        ax.set_xlabel("Social Status (z-scored ELO)", fontsize=13)
        ax.set_ylabel(f"P({outcome_labels[outcome]})", fontsize=13)
    ...
    # Save the plot
    save_path = os.path.join(OUTPUT_DIR, "social_status_effects_by_outcome.png")
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
...
def main():
    ...
    # Create individual social status plots from selected models (2x2 grid)
