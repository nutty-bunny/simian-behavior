...
# After saving the modeling data, add this section to generate forest plots
import matplotlib.pyplot as plt
import seaborn as sns

# Create a directory for plots if it doesn't exist
plot_dir = Path(base) / "derivatives" / "plots"
plot_dir.mkdir(parents=True, exist_ok=True)

def create_forest_plot(model, outcome_name):
    """Generate a forest plot for a GEE model"""
    ...
    plt.scatter(plot_data['coef'], plot_data['variable'], color='black', s=100)
    plt.hlines(y=plot_data['variable'], xmin=plot_data['lower'], xmax=plot_data['upper'], 
    ...
    plt.title(f'GEE Model: {outcome_name.capitalize()}\n'
              f'Variables: Species (Tonkean), Gender (Male=2), Age, ELO Score')
    
    # Save the plot
    plot_path = plot_dir / f"forest_plot_{outcome_name}.png"
    plt.tight_layout()
    plt.savefig(plot_path, dpi=300)
    plt.close()
    print(f"Saved forest plot for {outcome_name} to {plot_path}")

# Generate forest plots for all outcomes
for outcome, model in results.items():
    create_forest_plot(model, outcome)
